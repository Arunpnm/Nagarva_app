...
/// Daily operations and job tracking.
...
class _OperationsPageWidgetState extends State<OperationsPageWidget> {
  ...
  Future<void> _loadPendingApprovals() async {
    final rows = await OrdersTable().queryRows(
      queryFn: (q) =>
          OrgScope.read(q).eq('supervisor_status', 'completed_pending'),
    );
    _model.pendingApprovals = rows.toList().cast<OrdersRow>();
    safeSetState(() {});
  }

  Future<void> _approveEntry(OrdersRow order) async {
    // LEAK_AUDIT.md write-gap fix: matched only on id before.
    await OrdersTable().update(
      data: {'supervisor_status': 'approved'},
      matchingRows: (q) => OrgScope.write(q).eq('id', order.id!),
    );
  ...
  Future<void> _reopenEntry(OrdersRow order) async {
    // LEAK_AUDIT.md write-gap fix: matched only on id before.
    await OrdersTable().update(
            ...
            child: Padding(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    if (_model.pendingApprovals.isNotEmpty) ...[
                      Text(
                        'Pending Approvals (${_model.pendingApprovals.length})',
                                    ...
                                    style: FlutterFlowTheme.of(context)
                                        ...
                                        .override(
                                            font: GoogleFonts.inter(
                                                fontWeight: FontWeight.w600))),
                                Text(
                                    'Job completed, awaiting your review',
                                    style: FlutterFlowTheme.of(context)
                                        .bodySmall
                                        .override(
                                            font: GoogleFonts.inter(),
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        Builder(
                          builder: (context) {
                            final tripsViewListItem =
                                _model.tripsViewList.toList();
