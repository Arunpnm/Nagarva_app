...
-- ═══════════════════════════════════════════════════════════════════════════
--
-- -- Orders that still have job_team but no order_staff after this runs —
-- -- expect 0. (A non-zero row here most likely means a staff id inside
-- -- job_team no longer exists in `staff`, e.g. a deleted staff member.)
-- select o.id, o.job_team
-- from orders o
-- where o.job_team is not null
--   and jsonb_typeof(o.job_team) = 'array'
--   and jsonb_array_length(o.job_team) > 0
