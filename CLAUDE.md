...
- **A document is verified by RENDERING it and looking, never by
  reading the code.** (7 Sept 2026.) The Lorry Receipt printed as a
  letterhead and two address boxes — 29 words. The NOTICE, the freight
  breakdown, the total, the declaration and both signature lines were
  never drawn, and had not been for as long as the layout existed. The
  code read as correct, `flutter analyze` was clean, and a fixed
  `pw.Page` reports nothing when content does not fit.
  Cause: `pw.Row(crossAxisAlignment: stretch)` used to make three
  bordered panels share a height. **A horizontal Flex with `stretch`
  hands every child `maxHeight: constraints.maxHeight`, and inside a
  Column that is INFINITY** — the band claimed the whole sheet and its
  siblings had nowhere to go. Use `pw.Table` with
  `TableCellVerticalAlignment.full` instead: it measures the tallest
  cell and re-lays every cell to that height, bounded. A `pw.Column`
  with `stretch` is fine and unrelated — there stretch applies to width,
  which is bounded.
  The general rule: any change to a PDF gets rendered and read. The
  render harnesses (`test/document_terms_layout_test.dart`,
  `test/invoice_signature_layout_test.dart`) exist for exactly this and
  assert only that bytes are a PDF — the question they answer has no
  assertion. Two placement bugs in one pass were caught this way and
  would have shipped otherwise: terms rendered above BILL TO on the
  invoice (a `SizedBox` anchor matched the wrong one), and terms below
  the signature on the LR.
- **The platform's name must never appear on a vendor's document.**
  (Arun, 7 Sept 2026, extending the rule he set for the public survey
  and sign pages to printed output.) Three live instances: the shared
  PDF footer printed "Generated by Nagarva." on six document types; the
  POD fell back to `?? 'Nagarva'` for the LETTERHEAD when the session
  had no org name; and the quote/survey PDFs the customer downloads were
  named `Nagarva_Quote_...`. **Fall back to NOTHING, never to the
  platform** — an empty slot reads as plain, the wrong company reads as
  the wrong company, and the customer cannot tell the document is
...
- **7 Sept 2026, standard terms on every document + two defects the
  ...
  - `kDefaultStandardTerms` + `DocumentBoilerplate.standardTerms` +
    `PdfBranding.termsBlock()`, rendered on invoice, proforma, money
    receipt, LR, POD, packing list, loading slip, vehicle condition and
    voucher. Tenant-editable via `app_settings` key `document_terms`.
    **The shipped set is OPERATIONAL statements, not a liability
    position** — a default indemnity or liability cap would be this app
    deciding a vendor's exposure without their lawyer, which is "No
    suggested money. Ever." applied to words. Arun or his CA should read
    them.
  - **The LR was printing as a letterhead and two address boxes** —
    see the new render-and-look convention above. Fixed. Its FOOTER
    still does not render (the page is full before it is reached);
    pre-existing, verified by rendering with and without every change in
    flight, and left alone rather than shrinking a legal document's
    signature area inside a layout fix.
  - **The quotation's terms page printed vendor instructions on the
    customer's copy** — "Add your terms & conditions in Settings to show
    them here", shown to the person deciding whether to book. Falls back
    to the standard terms; both empty renders blank.
  - **Three places wore the platform's name on the vendor's papers** —
    see the convention above.
  - `PodPdf.generateForOrder` accepted a `terms` parameter its only
    caller never passed.
